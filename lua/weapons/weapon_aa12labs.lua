...
SWEP.PrintName = [[AA-12 Gen 2 "Labs"]]
SWEP.Author = "Unknown..."
SWEP.Instructions = "The AA-12 (Auto Assault - 12), originally designed and known as the Atchisson Assault Shotgun, is a fully automatic combat shotgun chambered in 12/70. This Gen 2 was created for a certain foreign PMC operating in the Norvinsk region.."
...
function SWEP:OnCantReload()
	...
	self.Inspecting = CurTime() + 5.5
	if self:Clip1() >= 1 then
	self:PlayAnim("check_1",5.5,false,function(self)
	self:PlayAnim("idle",1)
		--self.Inspecting = false
end,false,true)
else
	self:PlayAnim("check_1_empty_empty",2.5,false,function(self)
...
SWEP.FakeEmptyReloadSounds = {
	...
	[0.45] = "weapons/arccw_ur/ak/magrelease.ogg",
	[0.44] = "weapons/arccw_ur/ak/12ga/magout.ogg",
	[0.6] = "weapons/ak74/ak74_magout_rattle.wav",
	[0.83] = "weapons/arccw_ur/ak/12ga/magin.ogg",
...
local vector_full = Vector(1,1,1)
	SWEP.FakeReloadEvents = {
		...
		[0.46] = function(self,timeMul)
			if self:Clip1() < 1 then
			self:GetWM():SetBodygroup(5,0)
			self:GetWM():SetBodygroup(8,0)
			self:GetWM():ManipulateBoneScale(116, vector_origin)
			hg.CreateMag( self, Vector(7,0,0) )
			end
		end,
		[0.62] = function(self,timeMul)
				if self:Clip1() < 1 then
				self:GetWM():SetBodygroup(5,2)
				self:GetWM():SetBodygroup(8,1)
				UpdateVisualBullets(self:GetWM(),20)
				self:GetWM():ManipulateBoneScale(116, vector_full)
...
SWEP.WepSelectIcon2 = Material("entities/eft_aa12_attachments/gen2.png")
SWEP.WepSelectIcon2box = true
SWEP.IconOverride = "entities/eft_aa12_attachments/gen2.png"
...
SWEP.HoldType = "rpg"
SWEP.ZoomPos = Vector(0, 0.2208, 9.5363)
SWEP.RHandPos = Vector(-15, -2, 4)
SWEP.LHandPos = Vector(7, -2, -2)
SWEP.ShellEject = "ShotgunShellEject"
SWEP.SprayRand = {Angle(-0.2, -0.2, 0), Angle(0.2, 0.2, 0)}
...
SWEP.availableAttachments = {
	barrel = {
		[1] = {"supressor5", Vector(0.33,-0.25,0), {}},
	},
	sight = {
		["mountType"] = {"picatinny"},
		["mount"] = {["picatinny"] = Vector(-22.7, 3.1, 0.02)},
	},
		underbarrel = {
		[1] = {"laser5", Vector(0.0,0.4,0.2), {}},

		["mount"] = Vector(12.5, -2.2, -0.97),
		["mountAngle"] = Angle(-0.0, -0.3, 90),
