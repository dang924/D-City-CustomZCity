...
SWEP.Instructions = "The AK-105 is a short barrel, carbine version of the AK-74M rifle, originally developed to replace the shorter barrelled AKS-74U. The AK-105 is chambered in 5.45x39mm ammunition and is used domestically by the Russian Army in contrast to other AK-100 series rifles."
...
SWEP.FakeBodyGroups = "039000c2220"
...
function SWEP:ThinkAdd()
	if CLIENT and self:GetWM() and not isbool(self:GetWM()) and isstring(self.FakeBodyGroups) then
		if self:HasAttachment("grip", "grip_akdong") then
			self:GetWM():SetBodyGroups("029000c2220")
		else
...
SWEP.LocalMuzzlePos = Vector(19.985,-0.25,2.295)
...
SWEP.Spray = {}
for i = 1, 30 do
	SWEP.Spray[i] = Angle(-0.01 - math.cos(i) * 0.02, math.cos(i * i) * 0.02, 0) * 1.5
end

SWEP.WepSelectIcon2 = Material("vgui/entities/hud/weapon_tfa_dm_ak105")
SWEP.IconOverride = "vgui/entities/hud/weapon_tfa_dm_ak105"
...
SWEP.availableAttachments = {
	sight = {
		...
		["mount"] = {["dovetail"] = Vector(-17.5, 2.5, -0.45),["picatinny"] = Vector(-16.5, 3.02, -0.4)},
	},
	mount = {
		["picatinny"] = {
			"mount3",
			Vector(-13.5, 0.3, -1.35),
			{},
			["mountType"] = "picatinny",
		},
		["dovetail"] = {
	...
	barrel = {
		...
		[2] = {"supressor8", Vector(0.9,0,0), {}},
		["mount"] = Vector(0.4,0.1,-0.2),
	},
	grip = {
