...
SWEP.WorldModelFake = "models/weapons/csgo/c_pist_p250.mdl"
SWEP.FakePos = Vector(-16.5, 2.70, 3)
...
SWEP.FakeReloadSounds = {
	[0.30] = "zcitysnd/sound/weapons/m9/handling/m9_magout.wav",
...
SWEP.FakeEmptyReloadSounds = {
	...
	[0.85] = "zcitysnd/sound/weapons/m9/handling/m9_boltrelease.wav",
}
	...
	SWEP.FakeReloadEvents = {
		[0.32] = function( self ) 
			if self:Clip1() < 1 then
				hg.CreateMag( self, Vector(0,-30,10) )
				self:GetWM():ManipulateBoneScale(35, vector_origin)
			end
		end,
		[0.55] = function( self ) 
			if self:Clip1() < 1 then
				self:GetWM():ManipulateBoneScale(35, vector_full)
			end
		end,
		[0.36] = function( self ) 
			if self:Clip1() >= 1 then
...
SWEP.ZoomPos = Vector(0, 0.1384, 1.5854)
...
SWEP.availableAttachments = {
	barrel = {
		...
		[2] = {"supressor6", Vector(2.6,0.25,-0.4), {}},
		["mount"] = Vector(-0.0,0.4,0.3),
	},
		underbarrel = {
		[1] = {"laser5", Vector(0,0.2,0.35), {}},

		["mount"] = Vector(13, -0.2, -2.1),
		["mountAngle"] = Angle(-0.9, -0.15, 180),
...
function SWEP:DrawPost()
	...
	if CLIENT and IsValid(wep) then
		...
		wep:ManipulateBonePosition(43,Vector(0, 0,-0.6*self.shooanim ),false)
	end
end
...
function SWEP:AnimHoldPost()
	--self:BoneSet("r_finger0", vector_zero, finger6)	--self:BoneSet("l_finger0", vector_zero, finger1)
...
SWEP.LocalMuzzlePos = Vector(5.1,0.12,0.98)
SWEP.LocalMuzzleAng = Angle(0.7,-0.5,0)
