...
SWEP.Instructions = "The Zastava M70, formerly designated CZ M70 is a semi-automatic pistol produced by Zastava Arms as a sidearm for Yugoslav police and certain military officers. The pistol was loosely based on the Zastava M57, but is scaled down to accept the smaller and less powerful 7.65x17 mm"
SWEP.Category = "Weapons - Pistols"
SWEP.Slot = 2
SWEP.SlotPos = 10
SWEP.ViewModel = ""
SWEP.WorldModel = "models/weapons/w_pist_fiveseven.mdl"
SWEP.WorldModelFake = "models/weapons/zastavam70pistol.mdl"
SWEP.FakePos = Vector(-18.5, 2.5, 2.7)
...
SWEP.FakeEmptyReloadSounds = {
	[0.27] = "zcitysnd/sound/weapons/m9/handling/m9_magout.wav",
...
SWEP.lmagpos2 = Vector(0,-1.0,0)
SWEP.lmagang2 = Angle(90,0,-10)
...
if CLIENT then
	...
	SWEP.FakeReloadEvents = {
		...
		[0.32] = function( self ) 
			if self:Clip1() < 1 then
				hg.CreateMag( self, Vector(0,5,0) )
				self:GetWM():ManipulateBoneScale(59, vector_origin)
				self:GetWM():ManipulateBoneScale(60, vector_origin)
			end
		end,
		[0.52] = function( self ) 
			if self:Clip1() < 1 then
			self:GetWM():ManipulateBoneScale(59, vector_full)
...
SWEP.ZoomPos = Vector(0, 0.2406, 2.1029)
...
SWEP.attPos = Vector(-1.2, 0, 6.5)
SWEP.attAng = Angle(0, 90, 0)
SWEP.lengthSub = 25
...
SWEP.availableAttachments = {
	barrel = {
		...
		[2] = {"supressor6", Vector(1.7,0.55,0), {}},
		["mount"] = Vector(-0.6,0.4,0.02),
	}
}
...
function SWEP:DrawPost()
	...
	if CLIENT and IsValid(wep) then
		...
		wep:ManipulateBonePosition(58,Vector(-0.1*self.shooanim, -0.57*self.shooanim,0 ),false)
	end
end
...
SWEP.LocalMuzzlePos = Vector(1.7,0.27,1.57)
