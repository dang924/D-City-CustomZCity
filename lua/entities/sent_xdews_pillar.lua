...
function ENT:SetupDataTables()
    ...
    self:NetworkVar( "Bool", 1, "Free" ) // 默认解锁
    self:NetworkVar( "Bool", 2, "Persist" ) // 是否被地图保存
end
...
function ENT:Initialize()
    ...
    self:SetModel( GetConVar( "xdews_sv_altpillar" ):GetBool() and "models/xdews/pillar2.mdl" or "models/xdews/pillar.mdl" )
...
function ENT:RefreshData()
    ...
    if uid == "" then
        ...
        self:SetUID( "Pi_"..self:EntIndex().."_"..str )
...
function ENT:Use( act )
    ...
    if !self:GetActivated() then
        ...
        if !self:GetFree() and !act.xdews_unlocks[ self:GetUID() ] then
...
function ENT:OnTakeDamage( dmg )
    if !self:GetPersist() then self:TakePhysicsDamage( dmg ) end
end
...
function ENT:Draw()
    ...
    if !vgui.CursorVisible() and GetConVar( "xdews_cl_header" ):GetBool() and self:GetActivated() and !self.xdews_out and self:GetNick() != "" then
		if EyePos():DistToSqr( self:GetPos() ) <= 1024^2 and !LocalPlayer():ShouldDrawLocalPlayer() then
			...
			local p2, a2 = LocalToWorld( Vector( 0, 0, 60 ), Angle( 0, yy.yaw -90, 90 ), p1, Angle( 0, 0, 0 ) )
