...
function ENT:DoParticle()
	...
	if self.Burn == 74 then self:StopLoopingSound(self.snd or 0) end
	ParticleEffect("pcf_jack_smokebomb3", pos, -vector_up:Angle())
end

function ENT:Explode()
	self.Exploded = true
	self.snd = self:StartLoopingSound("weapons/m18/m18_burn_loop.wav")
	--self:DoParticle()
	timer.Create("smokenade" .. self:EntIndex(), 1, 75, function()
