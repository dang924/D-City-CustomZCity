...
local function draw_sprites()
	...
	do
		...
		if IsValid(wep) and wep:GetClass() == "weapon_afak_sh" then
			local mode = tonumber(wep.mode) or 1
			local modeToLimb = {
				[1] = "head",
				[2] = "torso",
				[3] = "left_arm",
	...
	for _, limb in ipairs(limbs) do
		...
		if selectedLimb == limb.name and dmg > 0.01 then
			local severity = math_min(math_max(dmg, 0), 1)
			local period = 2.4 - severity * 1.95
			local onWindow = 0.7 - severity * 0.4
			local phase = (CurTime() % period) / period

			if phase <= onWindow then
				local pulse = 1 - math_abs((phase / onWindow) * 2 - 1)
				local blend = 0.55 + pulse * 0.35
				col = Color(
					math_floor(Lerp(blend, col.r, 50)),
					math_floor(Lerp(blend, col.g, 255)),
					math_floor(Lerp(blend, col.b, 80)),
					255
				)
			end
		end
