...
if not hg or not hg.ammotypes then
    ErrorNoHalt("[9.3x64mm Brenneke] Ошибка: основная система аммо не найдена!\n")
    return
end
...
local function registerbrenneke()
    -- Добавляем в game.GetAmmoTypes
    game.AddAmmoType(hg.ammotypes["9.3x64mmbrenneke"])
    
    -- Регистрируем энтити
    if CLIENT then
        -- ВАЖНО: имя для language.Add должно совпадать с name из таблицы + "_ammo"
        language.Add("9.3x64mm Brenneke_ammo", "9.3x64mm Brenneke")
    end
    
    local ammoent = {}
    ammoent.Base = "ammo_base"
    ammoent.PrintName = "9.3x64mm Brenneke"
    ammoent.Category = "ZCity Ammo"
    ammoent.Spawnable = true
    ammoent.AmmoCount = hg.ammoents["9.3x64mmbrenneke"].Count or 20
    -- ВАЖНО: AmmoType должно быть точно таким же, как name в таблице ammotypes
    ammoent.AmmoType = "9.3x64mm Brenneke"
    ammoent.Model = "models/props_lab/box01a.mdl"
    ammoent.ModelMaterial = hg.ammoents["9.3x64mmbrenneke"].Material or ""
    ammoent.ModelScale = hg.ammoents["9.3x64mmbrenneke"].Scale or 1.1
    ammoent.Color = hg.ammoents["9.3x64mmbrenneke"].Color or Color(218, 165, 32)
    
    scripted_ents.Register(ammoent, "ent_ammo_9.3x64mmbrenneke")
    
    -- Перестраиваем типы аммо
    game.BuildAmmoTypes()
    
    print("[9.3x64mm Brenneke] Успешно загружен!")
end

if game.IsDedicated() or game.GetMap() ~= "" then
    registerbrenneke()
else
    hook.Add("Initialize", "init-ammo-brenneke", registerbrenneke)
end
...
if hg.ammotypesallowed then
    ...
    hg.ammotypesallowed["9.3x64mm Brenneke"] = hg.ammotypes["9.3x64mmbrenneke"]
end
...
if hg.ammotypeshuy then
    ...
    hg.ammotypeshuy["9.3x64mm Brenneke"] = table.Copy(hg.ammotypes["9.3x64mmbrenneke"])
    hg.ammotypeshuy["9.3x64mm Brenneke"].name = "9.3x64mmbrenneke"
end

-- Создаем обратную связь для системы дропа
if SERVER then
    -- Добавляем хук для обработки создания энтити при дропе
    hook.Add("PlayerAmmoDrop", "brenneke_drop_fix", function(ply, ammotype, count)
        -- Если это наш калибр, убеждаемся что создается правильная энтити
        if ammotype == "9.3x64mmbrenneke" then
...
print("[9.3x64mm Brenneke] Файл загружен, используйте ключ '9.3x64mm Brenneke' в коде винтовки")
