...
local function RebuildTriggerProxies()
    ...
    if not IsValid(tr) then
        Dbg("Rebuild failed: no valid trigger selected")
        return
    end
    ...
    local mins, maxs = tr:WorldSpaceAABB()
...
local function EnsureLuaMapEndAndDisableNative(reason)
    ...
    local best = PickBestChangelevelTrigger()
    local bestTarget = IsValid(best) and ResolveManagedTownTarget(game.GetMap()) or ""
    if not IsValid(best) or bestTarget == "" then
        Dbg("Ensure failed: best trigger invalid or target empty")
        return
    end

    DisableNativeMapEnds()
    DisableNativeChangelevelTriggers()
    RebuildTriggerProxies()
