...
if not hg or not hg.ammotypes then
    ErrorNoHalt("[Gauss] Ошибка: основная система аммо не найдена!\n")
    return
end
...
hg.ammoents["gauss"] = {
    Model = "models/weapons/arc9/stalker2/sr_gauss/w_sr_gauss_mag.mdl",
    Scale = 0.95,
...
local function registerGauss()
    -- Добавляем в game.GetAmmoTypes
    game.AddAmmoType(hg.ammotypes["gauss"])
    
    -- Регистрируем энтити
    if CLIENT then
        -- ВАЖНО: имя для language.Add должно совпадать с name из таблицы + "_ammo"
        language.Add("Gauss_ammo", "Gauss")
    end
    
    local ammoent = {}
    ammoent.Base = "ammo_base"
    ammoent.PrintName = "Gauss"
    ammoent.Category = "ZCity Ammo"
    ammoent.Spawnable = true
    ammoent.AmmoCount = hg.ammoents["gauss"].Count or 10
    -- ВАЖНО: AmmoType должно быть точно таким же, как name в таблице ammotypes
    ammoent.AmmoType = "Gauss"
    ammoent.Model = "models/weapons/arc9/stalker2/sr_gauss/w_sr_gauss_mag.mdl"
    ammoent.ModelMaterial = hg.ammoents["gauss"].Material or ""
    ammoent.ModelScale = hg.ammoents["gauss"].Scale or 1.2
    ammoent.Color = hg.ammoents["gauss"].Color or Color(218, 165, 32)
    
    scripted_ents.Register(ammoent, "ent_ammo_gauss")
    
    -- Перестраиваем типы аммо
    game.BuildAmmoTypes()
    
    print("[Gauss] Успешно загружен!")
end

if game.IsDedicated() or game.GetMap() ~= "" then
    registerGauss()
else
    hook.Add("Initialize", "init-ammo-gauss", registerGauss)
end
...
if hg.ammotypesallowed then
    ...
    hg.ammotypesallowed["Gauss"] = hg.ammotypes["gauss"]
end
...
if hg.ammotypeshuy then
    ...
    hg.ammotypeshuy["Gauss"] = table.Copy(hg.ammotypes["gauss"])
    hg.ammotypeshuy["Gauss"].name = "gauss"
end

-- Создаем обратную связь для системы дропа
if SERVER then
    -- Добавляем хук для обработки создания энтити при дропе
    hook.Add("PlayerAmmoDrop", "gauss_drop_fix", function(ply, ammotype, count)
        -- Если это наш калибр, убеждаемся что создается правильная энтити
        if ammotype == "gauss" then
...
print("[Gauss] Файл загружен, используйте ключ 'gauss' в коде винтовки")
